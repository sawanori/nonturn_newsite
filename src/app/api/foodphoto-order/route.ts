...
export async function POST(req: NextRequest) {
  try {
    ...
    const text = [
      ...
      `電話打合せ: ${parsed.phoneCall ? "あり" : "なし"}`,
      `ロケハン: ${parsed.locationScout ? "あり" : "なし"}`,
      `カメラマン指名: ${parsed.namedPhotographer ? "あり" : "なし"}`,
      "",
      "【合計金額】",
      `内訳: ${breakdown.join(" / ")}`,
      ...
      "",
      ...(parsed.campaignCode ? [`キャンペーンコード: ${parsed.campaignCode}`] : []),
      ...(parsed.referralCode ? [`紹介コード: ${parsed.referralCode}`] : []),
      "",
      "━━━━━━━━━━━━━━━━━━━━━━",
